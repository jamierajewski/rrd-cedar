...
for st in R Q; do
	...
	for j in `seq 0 $((i-1))`; do		
		...
		if [[ $j -eq 0 ]]; then
			...
			DEF_LINE_AREA=${DEF_LINE_AREA}" AREA:${USER[$j]}_${queue}${COLOR[$(($j % ${#COLOR[@]}))]}:${USER[$j]}@${queue}"
		else
			SUM=${SUM}",${USER[$j]}_${queue},UN,0,${USER[$j]}_${queue},IF,+"
			DEF_LINE_AREA=${DEF_LINE_AREA}" STACK:${USER[$j]}_${queue}${COLOR[$(($j % ${#COLOR[@]}))]}:${USER[$j]}@${queue}"
		fi	     
	done
	HLINE=""
	HLINE2=""
